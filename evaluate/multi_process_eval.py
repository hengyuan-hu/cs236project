...
def run_eval(env_params, agent, num_game, num_proc, seed, verbose=True):
    ...
    with torch.no_grad(), utils.eval_mode(agent):
        while len(processes) > 0:
            ...
            batch_action = agent.act(batch_obs, eval_mode=True).numpy()
    ...
    scores = [float(v) for v in results.values()]
    return scores
