...
class ResNetEncoderConfig:
    ...
    norm_layer: str = "gnn"
    shallow: int = 0
    feat_mode: str = "flat"  # flat/proj
    out_norm: int = 0  # only used for proj mode
...
class ResNetEncoder(nn.Module):
    def __init__(self, obs_shape, cfg: ResNetEncoderConfig):
        ...
        if self.cfg.feat_mode.startswith("proj"):
            proj_dim = int(self.cfg.feat_mode[len("proj") :])
            proj = nn.Linear(512, proj_dim)
            if self.cfg.out_norm:
                norm = nn.LayerNorm(proj_dim)
                self.proj = nn.Sequential(proj, norm)
            else:
                self.proj = nn.Sequential(proj)

        self.repr_dim = self.forward(torch.rand(1, *self.obs_shape)).size(1)
        self.patch_repr_dim = 512

    def forward(self, obs, flatten=True):
        obs = obs / 255.0 - 0.5
        h = self.nets(obs)
        if self.cfg.feat_mode == "flat":
            if flatten:
                h = h.flatten(1, -1)
            else:
                # convert to [batch, num_patch, dim] just to be consistent with RL
                h = h.flatten(2, 3).transpose(1, 2)
        elif self.cfg.feat_mode.startswith("proj"):
            h = h.flatten(2, 3).transpose(1, 2)
            # h: [batch, 3*3, 512]
            h = self.proj(h).sum(1)
        else:
